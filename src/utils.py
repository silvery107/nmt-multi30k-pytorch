...
# def beam_search(model, src, src_mask, max_len, start_symbol, end_symbol, device, beam_k=5):
#     src = src.to(device)
#     src_mask = src_mask.to(device)
#     memory = model.encode(src, src_mask).to(device)
#     answers = []
...
#     answers.append(torch.cat([next_prob.view(1,1,-1),next_word.view(1,1,-1)],dim=0))
#     next_word = next_word.item()
#     ys = torch.cat([bos, torch.ones(1, 1).type_as(src.data).fill_(next_word)], dim=0)
...
#                     continue
#                 # update ys and predict again
#                 ys = torch.cat([bos,seq],dim=1).type_as(src.data).transpose(0, 1)
#                 prob = get_prob(model, ys, memory, device) # prob: (1,len(en_vocab))
#                 # get top k ans
#                 next_k_prob, next_k_word = torch.topk(prob, beam_k, dim=1)
#                 # gen k new ans
...
#         _, top_k_idx = torch.topk(beam_score, beam_k, dim=0)
#         answers = [answers[i] for i in top_k_idx]
#         if all([ans[1, 0, -1]==end_symbol or len(ans[1, 0])>max_len for ans in answers]):
#             break
